...
if __name__ == '__main__':
    ...
    log_warning(f"Buscando 'y' no local: \033[m{local_scope.find('y')}") # Deve achar no local
    log_warning(f"Buscando 'x' no local: \033[m{local_scope.find('x')}") # Deve achar no global (via `previous`)
    ...
    log_warning(f"Buscando 'x' no local:  \033[m{local_scope.find('x')}")  # Deve ser char (local)
    log_warning(f"Buscando 'x' no global: \033[m{global_scope.find('x')}") # Deve ser int (original)
    #endregion
    #endregion [test]
